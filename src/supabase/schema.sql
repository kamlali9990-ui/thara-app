...
-- RPC لحساب رسوم التوصيل من الخادم (مصدر واحد للحقيقة)
...
BEGIN
  ...
  -- --- التحقق من تسلسل الحالة (server-side) ---
  IF p_status != 'ملغي' THEN
    ...
    END IF;
    -- يُسمح بالانتقال للأمام أو خطوة واحدة للخلف
    IF ni < ci - 1 THEN
      RAISE EXCEPTION 'لا يمكن إرجاع الطلب أكثر من خطوة واحدة';
    END IF;
  END IF;

  -- --- إذا تم الإلغاء، أعد المخزون ---
