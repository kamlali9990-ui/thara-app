...
const STATUS_LABELS: Record<string, { text: string; icon: string }> = {
...
const OrdersTab = memo(({ orders, loadOrders }: OrdersTabProps) => {
  ...
  const formatETA = (minutes: number | string): string | null => {
  ...
  const orderChatMsgs = (orderId: string) => chatMessages.filter(m => !m.orderId || m.orderId === orderId);
          ...
          <div key={order.id} className={`order-card-mini ${isExpanded ? 'expanded' : ''}`} onClick={() => setExpandedOrder(isExpanded ? null : order.id)} style={{ cursor: 'pointer' }}>
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: '0.75rem', marginTop: '0.25rem' }}>
                ...
                {order.status !== 'مكتمل' && order.status !== 'ملغي' && (
                  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
                    <button className="order-chat-btn" onClick={(e: React.MouseEvent) => { e.stopPropagation(); setChatOrder(order.id); }}>💬 محادثة الطلب</button>
                  </div>
                )}
              </div>
            )}
            ...
            {!isExpanded && (
              <div className="order-card-mini-total">
                {order.status !== 'مكتمل' && order.status !== 'ملغي' && (
                  <button className="order-chat-btn" onClick={(e: React.MouseEvent) => { e.stopPropagation(); setChatOrder(order.id); }} style={{ marginRight: 'auto' }}>💬 محادثة</button>
                )}
