...
function isValidBannerUrl(url: string | null | undefined): boolean {
  if (!url) return false;
...
const HomeTab = memo(({ addToCart, cart, searchQuery, setSearchQuery, setShowAllView }: HomeTabProps) => {
  ...
  useEffect(() => {
    (supabase.from('settings').select('value').eq('key', 'banner_url').maybeSingle() as unknown as Promise<any>)
      .then(({ data }: any) => {
      ...
      }).catch((e: any) => console.error('[banner fetch]', e));
    (supabase.from('settings').select('value').eq('key', 'featured_ids').maybeSingle() as unknown as Promise<any>)
      .then(({ data }: any) => {
        if (data?.value) {
        ...
        <>
          ...
          <div className="offers-ticker-container">
            <div className="offers-ticker-header" onClick={() => setShowAllView('offers')}>
              ...
              <span className="section-card-action-link" onClick={(e: React.MouseEvent) => { e.stopPropagation(); setShowAllView('offers'); }}>عرض الكل</span>
            </div>
            <div className="offers-ticker-track">
              {(allProducts || []).filter(p => p.isOffer).length > 0 ? 
                [...(allProducts || []).filter(p => p.isOffer), ...(allProducts || []).filter(p => p.isOffer)].slice(0, 12).map((product: any, idx: number) => (
                <div key={product.id + '-offer-' + idx} className="offer-ticker-item">
                  ...
                  <div className="offer-ticker-info">
                    ...
                    <button className="offer-ticker-add-btn" onClick={(e: React.MouseEvent) => { e.stopPropagation(); addToCart(product); }}>+</button>
                  </div>
                </div>
              )) : (
          ...
          {topStock.length > 0 && (
            <div className="offers-ticker-container">
              <div className="offers-ticker-header">
                ...
                <span className="section-card-action-link" onClick={(e: React.MouseEvent) => { e.stopPropagation(); setShowAllView('top-stock'); }}>عرض الكل</span>
              </div>
              <div className="offers-ticker-track">
                {topStock.map((product: any) => (
          ...
          <div className="home-section-card delivery-options-card" onClick={() => setShowDeliveryInfo(true)}>
            <div className="delivery-options-grid">
              <div className="delivery-option-item">
                ...
                <img src={`${BASE}newicon.jpg`} alt="توصيل" className="delivery-option-img" onError={(e: React.SyntheticEvent<HTMLImageElement>) => { e.currentTarget.style.display='none'; }} />
              </div>
            </div>
          </div>
          ...
          {showDeliveryInfo && (
            <div className="delivery-info-overlay" onClick={() => setShowDeliveryInfo(false)}>
              <div className="delivery-info-modal" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
                <button className="delivery-info-close" onClick={() => setShowDeliveryInfo(false)}>✕</button>
                <img src={`${BASE}newicon.jpg`} alt="توصيل" className="delivery-info-icon-img" onError={(e: React.SyntheticEvent<HTMLImageElement>) => { e.currentTarget.style.display='none'; }} />
