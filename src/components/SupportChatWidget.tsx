...
const SupportChatWidget = memo(() => {
  ...
  const supportMessages = useMemo(() => {
    if (!user) return [];
    return chatMessages.filter((m: ChatMessage) => !m.orderId && (m.customerEmail === user.email || (customerProfile?.phone && m.customerPhone === customerProfile.phone)));
  }, [chatMessages, user, customerProfile]);

  const unreadCount = useMemo(() => {
    if (!user || isOpen) return 0;
    const adminMsgs = chatMessages.filter((m: ChatMessage) => !m.orderId && (m.customerEmail === user.email || (customerProfile?.phone && m.customerPhone === customerProfile.phone)) && m.sender === 'admin' && m.status !== 'read');
    if (!lastOpenedSupport) return adminMsgs.length;
    return adminMsgs.filter((m: any) => {
  ...
  const StatusIcon = ({ status, failed }: { status?: string; failed?: boolean }) => {
          ...
          {!user ? (
            ...
            <>
              <div ref={chatBodyRef} className="chat-win-body">
                ...
                {supportMessages.map((m: ChatMessage, i: number) => {
