...
interface DiagnosticStep {
  ...
  fix: (() => boolean | Promise<boolean>) | null;
  chrome: string;
  safari: string;
  edge: string;
  pass: string;
...
const STEPS: DiagnosticStep[] = [
  ...
  },
  {
    ...
    check: async () => { try { const p = await navigator.permissions.query({ name: 'geolocation' as PermissionName }); return p.state === 'granted'; } catch { return false; } },
    fix: async () => { try { await new Promise<void>((res, rej) => navigator.geolocation.getCurrentPosition(() => res(), (e) => rej(e), { timeout: 2000 })); return true; } catch { return false; } },
...
export default function AdminDiagnostics() {
  ...
  const handleFix = async (step: DiagnosticStep) => {
    if (!step.fix) return;
    setFixing(step.id);
    try {
