...
const RotatingSection = memo(({ products, addToCart, setShowAllView, cart }: {
  ...
  const modes = useMemo(() => [
    {
      ...
      products: (allProducts || []).filter((p: any) => p.isOffer),
    },
    {
      id: 'all',
      icon: '🥬',
  ...
  const switchMode = (i: number) => { setModeIndex(i); setSubIndex(0); };
  const cycleMode = (e: any) => { e.stopPropagation(); switchMode((modeIndex + 1) % modes.length); };
