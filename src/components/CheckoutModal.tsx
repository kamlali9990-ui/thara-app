...
function getSavedCheckout(userEmail: string): any {
  try {
...
function saveCheckout(userEmail: string, data: any) {
  try {
...
interface CheckoutModalProps {
  cartTotal: number;
  onClose: () => void;
  placeOrder: (data: any, fee: number) => Promise<void>;
}

const CheckoutModal = memo<CheckoutModalProps>(({ cartTotal, onClose, placeOrder }) => {
  const { user, customerProfile, setUser } = useContext(StoreContext);
  const saved = user?.email ? getSavedCheckout(user.email) : {};
  const [position, setPosition] = useState<{ lat: number; lng: number } | null>(saved.position || null);
  ...
  const onLocateSuccess = (pos: GeolocationPosition) => {
  ...
  useEffect(() => {
    ...
    const doAutoLocate = async () => {
      ...
      if (navigator.permissions) {
        try {
          const p = await (navigator.permissions as any).query({ name: 'geolocation' });
          if (p.state === 'denied') {
            setIsLocating(true);
            const ok = await locateByIP();
            setIsLocating(false);
  ...
  useEffect(() => {
    ...
    ordersApi.getDeliveryFee(position.lat, position.lng, cartTotal).then((f: any) => {
      ...
      <div className="checkout-sheet" onClick={e => e.stopPropagation()}>
        ...
        <div className="checkout-body">
          <div className="checkout-section">
            ...
            {isOutsideService && (
              <div style={{textAlign:'center',padding:'0.85rem',background:'#fef3c7',borderRadius:'8px',marginBottom:'0.75rem',color:'#92400e',fontSize:'0.9rem',fontWeight:600}}>
                ⚠️ منطقتك خارج الخفجي — قد تتأخر التوصيلة ورسوم التوصيل 20 ر.س
              </div>
            )}
