...
function notifyNewOrder(order: any) {
  try {
...
function notifyNewMessage(msg: any) {
  try {
    if (!('Notification' in window)) return;
    if (Notification.permission !== 'granted') return;
...
function TabIcon({ name, badge, size = 22 }: { name: string; badge?: number; size?: number }) {
...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    const orderHandler = (e: Event) => { playNewOrderBeep(); notifyNewOrder((e as CustomEvent).detail); };
    window.addEventListener('thara:new-order', orderHandler as EventListener);
    const msgHandler = (e: Event) => { notifyNewMessage((e as CustomEvent).detail); };
    window.addEventListener('thara:new-message', msgHandler as EventListener);
    return () => {
      window.removeEventListener('thara:new-order', orderHandler as EventListener);
      window.removeEventListener('thara:new-message', msgHandler as EventListener);
    };
  }, []);

  const newOrdersCount = useMemo(() => orders.filter(o => o.status === 'جديد').length, [orders]);
  const unreadChatCount = useMemo(() => {
    return chatMessages.filter((m: any) => m.status !== 'read' && m.sender !== staffRole && m.sender !== 'system').length;
  }, [chatMessages, staffRole]);

  const tabs = useMemo(() => {
    const items: { id: string; label: string; icon: string; badge?: number }[] = [{ id: 'orders', label: 'الطلبات', icon: 'orders', badge: newOrdersCount }];
...
function FeaturedManager({ allProducts }: { allProducts: any[] }) {
  ...
  const featuredProducts = allProducts.filter((p: any) => featured.includes(p.id));
  const otherProducts = allProducts.filter((p: any) => !featured.includes(p.id) && (!searchQ || p.name.includes(searchQ)));
    ...
    <div className="admin-card" style={{ padding: '1.5rem' }}>
      ...
      {otherProducts.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem', maxHeight: '400px', overflowY: 'auto' }}>
          {otherProducts.slice(0, 100).map((p: any) => (
