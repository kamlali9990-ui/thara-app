-- Fix: robust location parsing (handle multiple formats)
-- Also fix function call syntax with OUT params
...
AS $$
  ...
  SELECT
    CASE
      -- Format: "Lat: XX.XXXX, Lng: YY.YYYY"
      WHEN s ~* 'Lat:' THEN
        NULLIF(TRIM(SPLIT_PART(SPLIT_PART(s, 'Lat:', 2), ',', 1)), '')::NUMERIC
      -- Format: "XX.XXXX,YY.YYYY" or "XX.XXXX, YY.YYYY"
      WHEN s ~ '^[\d\.\-]+[, ]+[\d\.\-]+$' THEN
        NULLIF(TRIM(SPLIT_PART(s, ',', 1)), '')::NUMERIC
      -- Format: JSON object {"lat":XX,"lng":YY}
      WHEN s ~* '"lat"' OR s ~* '"lng"' OR s ~* '"latitude"' THEN
        (s::json ->> 'lat')::NUMERIC
      ELSE NULL
    END AS lat,
    CASE
      WHEN s ~* 'Lng:' THEN
        NULLIF(TRIM(SPLIT_PART(s, 'Lng:', 2)), '')::NUMERIC
      WHEN s ~ '^[\d\.\-]+[, ]+[\d\.\-]+$' THEN
        NULLIF(TRIM(SPLIT_PART(s, ',', 2)), '')::NUMERIC
      WHEN s ~* '"lng"' OR s ~* '"lon"' OR s ~* '"longitude"' THEN
        (s::json ->> 'lng')::NUMERIC
      ELSE NULL
    END AS lng
  FROM input
...
-- Recreate create_order_secure with fixed function call syntax
...
BEGIN
  ...
  -- Parse location to calculate delivery fee (uses FROM syntax for OUT params)
